...
class FactoryBasedGeneratorTest {
    ...
    void testWithSpecificSeed(String value) {
        // This test will always generate the same values
        assertEquals("ErrorString", value);
    }
}
...
4. **`opens` directive**: Add `opens your.test.package;` to your `module-info.java` to allow reflective access
5. **Targeted `opens` directive**: Add `opens your.test.package to org.junit.platform.commons;` for more restricted access
6. **`--add-opens` JVM argument** (Gradle-compatible): Add `--add-opens your.module/your.test.package=ALL-UNNAMED` to the JVM arguments in your test configuration
7. **Use `@TypeGeneratorMethodSource`**: Define a factory method inside your test class that creates and returns the generator, avoiding external class reflection entirely
